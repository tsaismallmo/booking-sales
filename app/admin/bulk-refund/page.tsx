...
// A:訂單歸屬 | B:分店 | C:日期+星期 | D:時段 | E:人數 | F:訂位代號 | G:姓名+電話 | H:訂金 | I:備註
...
export default function BulkRefundPage() {
  ...
  const handleImport = async () => {
    ...
    setRows(updated);
    setImporting(false);
    setSummary(`完成：${ok} / ${valid.length} 筆標記為退訂`);
    setPhase("done");
  };
  ...
  return (
    <div className="erp-page">
      <div className="erp-page-header">
        <div>
          <h1 className="erp-page-title">批次更新退訂</h1>
          <p className="erp-page-subtitle">
            {phase === "input" && "從 Excel 複製資料貼上，系統自動比對單據，全部標記為「退」"}
        ...
        {phase === "preview" && (
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={handleImport} disabled={importing || matchCount === 0} className="btn btn-primary">
              {importing ? "更新中..." : `確認標記退訂（${matchCount} 筆）`}
            </button>
          </div>
        )}
        {phase === "done" && (
        ...
        <>
          <div className="erp-card" style={{ marginBottom: 16 }}>
            ...
            <div className="erp-card-body" style={{ fontSize: 13, color: "var(--gray-600)", lineHeight: 1.9 }}>
              ...
              <p><strong>方式二：沒有標題列</strong>（固定 9 欄，A–I）</p>
              <p style={{ marginLeft: 12, fontFamily: "monospace", fontSize: 12, background: "var(--gray-50)", padding: "4px 8px", borderRadius: 4, lineHeight: 1.7 }}>
                A:訂單歸屬 | B:分店 | C:<strong>日期+星期</strong> | D:時段 | E:人數 | F:訂位代號 | G:姓名+電話 | H:訂金 | I:備註
              </p>
              <p style={{ marginTop: 6 }}>
                · 日期可用 <code>2026-04-05</code>、<code>4/5</code> 或 <code>4月5日</code><br />
                · 比對到的單據會一律標記狀態為「退」，其他欄位不會被改動
              </p>
            </div>
          </div>
